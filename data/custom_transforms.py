...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            ...
            elif elem == 'image' or elem == 'strong' or  elem == 'augmented'or  elem == 'neighbor' :
                sample[elem] = self.to_tensor(sample[elem]) # Regular ToTensor operation 
                # print(1111)
            elif elem in ['semseg', 'sal', 'semseg_10','semseg_aug','semseg_nei','temp']:
                sample[elem] = torch.from_numpy(sample[elem]).long() # Torch Long
                # print(2222)
            elif elem in ['target']:
                # sample[elem] = torch.from_numpy(sample[elem]).long() # Torch Long
                sample[elem] = torch.tensor(sample[elem] , dtype=torch.int8).long()
            else:
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        sample['augmented'] = self.normalize(sample['augmented'])
        sample['neighbor'] = self.normalize(sample['neighbor'])
        if 'strong' in sample:
...
class ColorJitter(object):
    ...
    def __call__(self, sample):
        ...
        sample['augmented'] = self.jitter(sample['augmented']) 
        sample['neighbor'] = self.jitter(sample['neighbor']) 
        return sample
...
class RandomHorizontalFlipPIL(object):
    def __call__(self, sample):
        ...
        if random.random() < 0.5:
            ...
            sample['augmented'] = sample['augmented'].transpose(Image.FLIP_TOP_BOTTOM)
            sample['neighbor'] = sample['neighbor'].transpose(Image.FLIP_LEFT_RIGHT)
            if 'sal' in sample:
                sample['sal'] = sample['sal'].transpose(Image.FLIP_LEFT_RIGHT)
            if 'semseg' in sample:
                sample['semseg'] = sample['semseg'].transpose(Image.FLIP_LEFT_RIGHT)
                sample['semseg_aug'] = sample['semseg_aug'].transpose(Image.FLIP_TOP_BOTTOM)
                sample['semseg_nei'] = sample['semseg_nei'].transpose(Image.FLIP_LEFT_RIGHT)
...
class RandomGrayscale(object):
    ...
    def __call__(self, sample):
        img = sample['image']
        num_output_channels = 3 # if img.mode == 'L' else 3
...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        sample['augmented'] = self.resize_img(sample['augmented'])
        sample['neighbor'] = self.resize_img(sample['neighbor'])
        
        if 'sal' in sample:
            sample['sal'] = self.resize_sal(sample['sal'])
        if 'semseg' in sample:
            # print('sa',sample['semseg'].shape)
            sample['semseg']=sample['semseg'].unsqueeze(0)
            sample['semseg'] = self.resize_sal(sample['semseg'])
            sample['semseg']=sample['semseg'].squeeze(0)

            sample['semseg_aug']=sample['semseg_aug'].unsqueeze(0)
            sample['semseg_aug'] = self.resize_sal(sample['semseg_aug'])
            sample['semseg_aug']=sample['semseg_aug'].squeeze(0)


            sample['semseg_nei']=sample['semseg_nei'].unsqueeze(0)
            sample['semseg_nei'] = self.resize_sal(sample['semseg_nei'])
            sample['semseg_nei']=sample['semseg_nei'].squeeze(0)
        # sys.exit()
        return sample
import sys
class RandomResizedCrop(T.RandomResizedCrop):
    ...
    def __call__(self, sample):
        ...
        sample['augmented'] = F.resized_crop(img, i, j, h, w, self.size, self.interpolation_sal)
        sample['neighbor'] = F.resized_crop(img, i, j, h, w, self.size, self.interpolation_img)
        ...
        if 'semseg' in sample:
            ...
            sample['semseg'] = sample['semseg'].unsqueeze(0)
            # print('semseg',semseg.shape,type(semseg))
            sample['semseg'] = F.resized_crop(sample['semseg'], i, j, h, w, self.size, self.interpolation_img).squeeze(0)

            sample['semseg_aug'] = sample['semseg_aug'].unsqueeze(0)
            # print('semseg',semseg.shape,type(semseg))
            sample['semseg_aug'] = F.resized_crop(sample['semseg_aug'], i, j, h, w, self.size, self.interpolation_sal).squeeze(0)
            # print( sample['semseg'].shape)
            sample['semseg_nei'] = sample['semseg_nei'].unsqueeze(0)
            # print('semseg',semseg.shape,type(semseg))
            sample['semseg_nei'] = F.resized_crop(sample['semseg_nei'], i, j, h, w, self.size, self.interpolation_sal).squeeze(0)
